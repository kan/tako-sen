...
問題共有リンクは作成者のみログイン必須で、受け取った人は未ログインでも問題を開けます。盤面の公開スナップショットは正解配置を含まず、受信側で唯一解を検証します。開発用 Worker で未ログインの別端末から同じ盤面を開いてクリアし、ログイン後に履歴を取り込めることを確認済みです。再送しても同じ挑戦は重複しません。

オンライン履歴の表示では、取得済みの直近500件から問題別自己ベスト、期間・難易度別集計、最近のクリア時間、シード・問題 ID の絞り込みを行います。開発用 Worker で表示と絞り込みの動作を確認済みです。これらは本人だけの非公開集計であり、全ユーザーの公開ランキングではありません。

### 公開前の確認事項

公開ランキングは2026-09-27に開発用 D1 へ `0003_public_leaderboards.sql` を適用し、Worker にデプロイ済みです。記録の公開と別アカウントからの閲覧を確認しました。「オンライン履歴」の「該当する履歴」から記録ごとに公開／取り消しを選び、「この問題の公開ランキング」で未ログインでも比較できます。既存履歴は公開されません。自動生成の匿名名を使い、自己申告の参考順位として扱います。「すべてのランキング公開を取り消す」は直近500件より古い記録も対象です。現行の同期・ランキング操作は複雑なため、β公開前に大幅な UX 見直しを行います（[`docs/NEXT_PHASE.md`](docs/NEXT_PHASE.md)）。
