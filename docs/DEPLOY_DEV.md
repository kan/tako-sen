...
2026-09-27に承認を得て `0003_public_leaderboards.sql` を開発用 D1 に適用し、Worker を再デプロイした。バージョン ID は `69346364-7a6e-49e0-9ece-75e856e7ed15`。既存記録の `is_public` は既定で `0` となり、適用だけでは公開されない。利用者が記録の公開と別アカウントからの閲覧を確認した。未ログイン閲覧・ベスト1件の選択・公開取り消し・公開後の退会については実機確認を継続する。同期とランキングの操作が複雑なため、β公開前に UX を大幅に見直す（`NEXT_PHASE.md`）。

デプロイ直後の確認: 既存履歴4件・公開0件。静的画面と未認証のランキング取得は `200`、不正な問題 ID は `400`、未認証の履歴取得・個別公開・全件公開取り消しは `401`。ランキング応答は `no-store`。

1. ログインして「オンライン履歴を更新」。既存履歴が非公開であることを確認する。
2. テスト用記録を1件だけ公開し、その問題の共有リンクを未ログインの別ブラウザで開く。「この問題の公開ランキング」を更新し、匿名名と成績だけが見えることを確認する。
3. 同じ問題の別記録を公開しても同一参加者は1人になることを確認する。個別の公開取り消しでは他の公開済み記録が残る場合がある。
4. 「すべてのランキング公開を取り消す」を実行し、別ブラウザのランキングを更新すると消えること、オンライン履歴は残ることを確認する。
5. 退会確認が必要な場合は使い捨てアカウントだけを使う。公開記録・匿名名の削除と、端末のローカル履歴が残ることを確認する。
